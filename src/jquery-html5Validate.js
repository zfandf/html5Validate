 ...
 * Update by zhangff on 2015-12-21
 *     1. 表单添加 data-type 属性，非表单自带type或者多类型校验，使用data-type="phone|email"
 *     2. 删除 $.html5Attr 方法，使用 $.attr() 替换
 *     3. 添加校验 PHONE，    
 * 
 **/
(function ($, undefined) {
    ...
    var OBJREG = {
        ...
        TEL: "^1\\d{10}$", // 
        ZIPCODE: "^\\d{6}$", // 邮编校验
        PHONE: "^(0|\\+86)?1[34578][0-9]{9}$", // 手机号码校验 支持格式 11位手机号，"0"+11位手机号， "+86"+11位手机号
        prompt: {
            radio: "请选择一个选项",
        ...
        return {
            ...
            isRegex: function (ele, regex, params) {
                ...
                var inputValue = ele.value, 
                    ...
                    type = $(ele).attr('data-type') || ele.getAttribute("type") + "";
            ...
            isAllpass: function (elements, options) {
                        ...
                        if (params.labelDrive) {
                            ...
                            label.each(function () {
                                ...
                                if (txtLabel !== placeholder) {
                                    text += txtLabel.replace(/\*|:|：/g, ""); // 去除 "姓名："中的":"
                                }
                            });
                        }
                ...
                elements.each(function () {
                    var el = this, 
                        type = $(el).attr('data-type') || el.getAttribute("type"), 
                    ...
                    if (type == "radio" && isRequired) {
                        ...
                        allpass ? remind(el, type, "empty") : remind(el, type, tag); // TODO allpass不是整体是否通过么，如果为true,
                        allpass = false;
        ...
        html5Validate: function (callback, options) {
            ...
            if ($.html5Validate.isSupport) {
                if (params.novalidate) {
                    ...
                    params.hasTypeNormally = true; // TODO hasTypeNormally， 支持HTML5 并且html5自动检测打开
                }
            }
